...
def canon(
    year, brand, set_name, subset, card_no, parallel, variant
) -> str:
    def n(v): return ("" if v is None else str(v)).strip().lower()
    return "|".join([n(year), n(brand), n(set_name), n(subset), n(card_no), n(parallel), n(variant)])
...
def create_card(payload: CardCreate, db: Session = Depends(get_db)):
    card = Card(
        ...
        team=payload.team, sport=payload.sport, parallel=payload.parallel, variant=payload.variant,
        print_run=payload.print_run, notes=payload.notes,
    )
    card.canonical_key = canon(card.year, card.brand, card.set_name, card.subset, card.card_no, card.parallel, card.variant)
    db.add(card)
    db.commit()
    db.refresh(card)
    return card
...
def update_card(card_uuid: str, payload: CardUpdate, db: Session = Depends(get_db)):
    card: Card = db.query(Card).filter(Card.card_uuid == card_uuid, Card.deleted_at.is_(None)).first()
    if not card:
